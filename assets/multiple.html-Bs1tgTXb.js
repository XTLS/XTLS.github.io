import{_ as s,r as a,o,c as t,a as p,e}from"./app-DAbIUvv6.js";const c={},l=e(`<h1 id="多文件配置" tabindex="-1"><a class="header-anchor" href="#多文件配置"><span>多文件配置</span></a></h1><p>Xray 程序支持使用多个配置文件。</p><p>多配置文件的主要作用在于分散不同作用模块配置，便于管理和维护。</p><p>该功能主要考虑是为了丰富 Xray 的生态链，比如对于 GUI 的客户端，一般只实现节点选择等固定的功能，对于太复杂的配置难以图形化实现；只需留一个 <code>confdir</code> 的自定义配置目录供配置复杂的功能；对于服务器的部署脚本，只需往 <code>confdir</code> 添加文件即可实现配置多种协议。</p><h2 id="多文件启动" tabindex="-1"><a class="header-anchor" href="#多文件启动"><span>多文件启动</span></a></h2><div class="custom-container tip"><p class="custom-container-title">提示</p><p>启动信息中会提示依次读入的每个配置文件，留意启动信息是否符合你预设的顺序。可以在每个文件名前面增加前缀数字的方式控制顺序。如 <code>01_文件名</code>, <code>02_文件名</code>，数字越大排序越靠后。</p></div><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ xray run <span class="token parameter variable">-confdir</span> /etc/xray/confs
