import{_ as o,r as p,o as i,c as l,a as n,b as s,d as e,e as t}from"./app-JqiKuxQm.js";const c="/assets/xray-fallbacks-IC5ujKgM.svg",r="/assets/xray-dns-records-JaWruCUz.webp",u="/assets/cf-api-token-permissions-for-acme-Zhny1POg.webp",d={},v=t(`<h1 id="通过-sni-回落功能实现伪装与按域名分流" tabindex="-1"><a class="header-anchor" href="#通过-sni-回落功能实现伪装与按域名分流" aria-hidden="true">#</a> 通过 SNI 回落功能实现伪装与按域名分流</h1><p>VLESS 是一种很轻的协议，和 Trojan 一样，不对流量进行复杂的加密和混淆，而是大隐隐于市，通过 TLS 协议加密，混杂在其他 HTTPS 流量中，在墙内外穿进穿出。为了更好的伪装以应对主动探测，Fallbacks 回落功能随 VLESS 同时出现。这篇教程将演示如何使用 Xray 中 VLESS 入站协议的回落功能配合 Nginx 或 Caddy 在保证伪装完全的前提下实现按域名分流。</p><h2 id="应用情景" tabindex="-1"><a class="header-anchor" href="#应用情景" aria-hidden="true">#</a> 应用情景</h2><p>由于 XTLS，Xray 需要监听 443 端口，这导致如果之前有网站运行在服务器上，那么此时网站无法运行或需要运行在其他端口上，这显然是不合理的。有以下三种方案可以解决这个问题：</p><ul><li><p>Xray 监听其他常用端口（如 22、3389、8443）</p><p>这个方案是最简单的，但不够完美。</p></li><li><p>Nginx 或 HAProxy 监听 443 端口，通过 SNI 分流做 L4 反向代理，实现端口复用</p><p>这个方案比较复杂，需要对 Nginx 或 HAProxy 的使用有一定了解，此处不作过多解释。</p></li><li><p>Xray 监听 443 端口，通过 Fallbacks 功能 SNI 分流将网站流量回落到 Nginx 或 Caddy</p><p>这个方案难度适中，也是此教程接下来想要演示的方案。</p></li></ul><h2 id="sni-简介" tabindex="-1"><a class="header-anchor" href="#sni-简介" aria-hidden="true">#</a> SNI 简介</h2><p>服务器名称指示（英语：<strong>S</strong>erver <strong>N</strong>ame <strong>I</strong>ndication，缩写：<strong>SNI</strong>）是 TLS 的一个扩展协议。熟悉反向代理的朋友都知道，如果想要通过域名将流量代理到正确的内容上，需要以下配置：</p><div class="language-nginx line-numbers-mode" data-ext="nginx"><pre class="language-nginx"><code><span class="token directive"><span class="token keyword">proxy_set_header</span> Host 主机名</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这句的作用是将名为 “Host” 的 HTTP Header 设定为某个主机名。为什么要这样做？一般而言，一台服务器对应一个 IP，但却运行多个网站，访问者通过域名查询到 IP 以访问服务器，那么问题来了，如何确定访问者想要访问的是哪一个网站？这需要“基于名称的虚拟主机”。</p><p>当 Web 服务器收到访问请求后，它会查看请求的主机头，使访问者访问正确的网站。然而当 HTTP 协议被 TLS 协议加密后，这种简单的方法就无法实现了。因为 TLS 握手发生在服务器看到任何 HTTP 头之前，因此，服务器不可能使用 HTTP 主机头中的信息来决定呈现哪个证书，更无法决定访问者的访问目标。</p><p>SNI 的原理也很简单，它通过让客户端发送主机名作为 TLS 协商的一部分来解决此问题。所以在使用 Nginx 对 HTTPS 协议进行反向代理时，需要在配置中加入 <code>proxy_ssl_server_name on;</code>，此时 Nginx 会向被代理的服务器发送 SNI 信息，解决了 HTTPS 协议下虚拟主机失效的问题。另外，使用 SNI 时，即使不指定主机头，也可以正确访问网站。</p><h2 id="思路" tabindex="-1"><a class="header-anchor" href="#思路" aria-hidden="true">#</a> 思路</h2><p><img src="`+c+'" alt="Xray 回落流程"></p><p>从 443 端口接收到流量后，Xray 会把 TLS 解密后首包长度 &lt; 18、协议版本无效或身份认证失败的流量通过对 name、path、alpn 的匹配转发到 dest 指定的地址。</p><h2 id="添加-dns-记录" tabindex="-1"><a class="header-anchor" href="#添加-dns-记录" aria-hidden="true">#</a> 添加 DNS 记录</h2><p><img src="'+r+'" alt="DNS 记录"></p><p>请按实际情况修改域名和 IP。</p><h2 id="申请-tls-证书" tabindex="-1"><a class="header-anchor" href="#申请-tls-证书" aria-hidden="true">#</a> 申请 TLS 证书</h2>',18),k=n("code",null,"*.example.com",-1),m=n("code",null,"example.com",-1),b=n("code",null,"*.*.example.com",-1),h={href:"https://zh.wikipedia.org/wiki/%E4%B8%BB%E9%A2%98%E5%A4%87%E7%94%A8%E5%90%8D%E7%A7%B0",target:"_blank",rel:"noopener noreferrer"},q=n("sup",{class:"footnote-ref"},[n("a",{href:"#fn1",id:"fnref1"},"[1]")],-1),f={href:"https://acme.sh",target:"_blank",rel:"noopener noreferrer"},y={href:"https://github.com/acmesh-official/acme.sh/wiki/dnsapi",target:"_blank",rel:"noopener noreferrer"},g={href:"https://dash.cloudflare.com/profile/api-tokens",target:"_blank",rel:"noopener noreferrer"},_=t('<p><img src="'+u+`" alt="API Token 的权限设置"></p><p>权限部分至关重要，其他部分任意。</p><p>创建完毕后，你会得到一串神秘字符，请将其妥善保管到安全且不会丢失的地方，因为它不再会显示。这串字符就是即将用到的 <code>CF_Token</code>。</p><div class="custom-container tip"><p class="custom-container-title">注意</p><p>以下操作需要在 root 用户下进行，使用 sudo 会出现错误。</p></div><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">curl</span> https://get.acme.sh <span class="token operator">|</span> <span class="token function">sh</span> <span class="token comment"># 安装 acme.sh</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="xray-配置" tabindex="-1"><a class="header-anchor" href="#xray-配置" aria-hidden="true">#</a> Xray 配置</h2><div class="language-json line-numbers-mode" data-ext="json"><pre class="language-json"><code><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
  ...
  <span class="token punctuation">]</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li></ul><h2 id="nginx-配置" tabindex="-1"><a class="header-anchor" href="#nginx-配置" aria-hidden="true">#</a> Nginx 配置</h2><p>Nginx 将通过官方源进行安装。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">sudo</span> <span class="token function">apt</span> <span class="token function">install</span> <span class="token function">curl</span> gnupg2 ca-certificates lsb-release
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>删除 <code>/etc/nginx/conf.d/default.conf</code> 并创建 <code>/etc/nginx/conf.d/fallbacks.conf</code>，内容如下：</p><div class="language-nginx line-numbers-mode" data-ext="nginx"><pre class="language-nginx"><code><span class="token directive"><span class="token keyword">set_real_ip_from</span> 127.0.0.1</span><span class="token punctuation">;</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="caddy-配置" tabindex="-1"><a class="header-anchor" href="#caddy-配置" aria-hidden="true">#</a> Caddy 配置</h2>`,15),x={href:"https://caddyserver.com/docs/install",target:"_blank",rel:"noopener noreferrer"},T=t(`<p>为了使 Caddy 能获取到访问者的真实 IP，需要编译带有 Proxy Protocol 模块的 Caddy。建议直接在 Caddy 官网上在线编译。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">sudo</span> <span class="token function">curl</span> <span class="token parameter variable">-o</span> /usr/bin/caddy <span class="token string">&quot;https://caddyserver.com/api/download?os=linux&amp;arch=amd64&amp;p=github.com%2Fmastercactapus%2Fcaddy2-proxyprotocol&amp;idempotency=79074247675458&quot;</span>
<span class="token function">sudo</span> <span class="token function">chmod</span> +x /usr/bin/caddy
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>直接替换即可。</p><div class="custom-container tip"><p class="custom-container-title">提示</p><p>建议先通过官网文档安装 Caddy，再替换二进制文件。这样做无需手动设定进程守护。</p></div><p>编辑 <code>/etc/caddy/Caddyfile</code>：</p><div class="language-Caddyfile line-numbers-mode" data-ext="Caddyfile"><pre class="language-Caddyfile"><code>{
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="参考" tabindex="-1"><a class="header-anchor" href="#参考" aria-hidden="true">#</a> 参考</h2>`,7),P={href:"https://zh.wikipedia.org/wiki/%E6%9C%8D%E5%8A%A1%E5%99%A8%E5%90%8D%E7%A7%B0%E6%8C%87%E7%A4%BA",target:"_blank",rel:"noopener noreferrer"},w={href:"https://github.com/acmesh-official/acme.sh/wiki",target:"_blank",rel:"noopener noreferrer"},S={href:"https://zh.wikipedia.org/wiki/HTTP/2",target:"_blank",rel:"noopener noreferrer"},N=n("h2",{id:"引用",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#引用","aria-hidden":"true"},"#"),s(" 引用")],-1),H=n("hr",{class:"footnotes-sep"},null,-1),L={class:"footnotes"},C={class:"footnotes-list"},I={id:"fn1",class:"footnote-item"},E={href:"https://letsencrypt.org/zh-cn/docs/faq/",target:"_blank",rel:"noopener noreferrer"},A=n("a",{href:"#fnref1",class:"footnote-backref"},"↩︎",-1),j={id:"fn2",class:"footnote-item"},D={href:"https://www.haproxy.com/blog/haproxy/proxy-protocol/",target:"_blank",rel:"noopener noreferrer"},B=n("a",{href:"#fnref2",class:"footnote-backref"},"↩︎",-1),X={id:"fn3",class:"footnote-item"},F={href:"https://www.jianshu.com/p/cc8d592582c9",target:"_blank",rel:"noopener noreferrer"},V=n("a",{href:"#fnref3",class:"footnote-backref"},"↩︎",-1),z={id:"fn4",class:"footnote-item"},U={href:"https://github.com/rprx/v2fly-github-io/blob/master/docs/config/protocols/vless.md",target:"_blank",rel:"noopener noreferrer"},W=n("a",{href:"#fnref4",class:"footnote-backref"},"↩︎",-1);function R($,J){const a=p("ExternalLinkIcon");return i(),l("div",null,[v,n("p",null,[s("由于要对不同前缀的域名进行分流，但一个通配符证书的作用域仅限于两“.”之间（例如：申请 "),k,s("，"),m,s(" 和 "),b,s(" 并不能使用该证书），故需申请 "),n("a",h,[s("SAN"),e(a)]),s(" 通配符证书。根据 Let's Encrypt 官网信息"),q,s("，申请通配符证书要求 DNS-01 验证方式，此处演示 NS 记录为 Cloudflare 的域名通过 "),n("a",f,[s("acme.sh"),e(a)]),s(" 申请 Let's Encrypt 的免费 TLS 证书。使用其他域名托管商的申请方法请阅读 "),n("a",y,[s("dnsapi · acmesh-official/acme.sh Wiki"),e(a)]),s("。")]),n("p",null,[s("首先需要到 "),n("a",g,[s("Cloudflare 面板"),e(a)]),s("创建 API Token。参数如下：")]),_,n("p",null,[s("安装 Caddy 请参阅 "),n("a",x,[s("Install — Caddy Documentation"),e(a)]),s("。")]),T,n("ol",null,[n("li",null,[n("a",P,[s("服务器名称指示 - 维基百科，自由的百科全书"),e(a)])]),n("li",null,[n("a",w,[s("Home · acmesh-official/acme.sh Wiki"),e(a)])]),n("li",null,[n("a",S,[s("HTTP/2 - 维基百科，自由的百科全书"),e(a)])])]),N,H,n("section",L,[n("ol",C,[n("li",I,[n("p",null,[n("a",E,[s("常见问题 - Let's Encrypt - 免费的 SSL/TLS 证书"),e(a)]),s(),A])]),n("li",j,[n("p",null,[n("a",D,[s("Proxy Protocol - HAProxy Technologies"),e(a)]),s(),B])]),n("li",X,[n("p",null,[n("a",F,[s("proxy protocol 介绍及 nginx 配置 - 简书"),e(a)]),s(),V])]),n("li",z,[n("p",null,[n("a",U,[s("v2fly-github-io/vless.md at master · rprx/v2fly-github-io"),e(a)]),s(),W])])])])])}const M=o(d,[["render",R],["__file","fallbacks-with-sni.html.vue"]]);export{M as default};
