import{at as e,it as t,t as n,vt as r}from"./chunks/framework.B8HA7oqK.js";var i=JSON.parse(`{"title":"Nginx 或 Haproxy 搭建 TLS 隧道隐藏指纹","description":"","frontmatter":{},"headers":[],"relativePath":"document/level-2/nginx_or_haproxy_tls_tunnel.md","filePath":"document/level-2/nginx_or_haproxy_tls_tunnel.md","lastUpdated":1783775751000}`),a={name:`document/level-2/nginx_or_haproxy_tls_tunnel.md`};function o(n,i,a,o,s,c){return r(),t(`div`,null,[...i[0]||=[e(`<div style="display:none;" hidden="true" aria-hidden="true" data-nosnippet>Are you an LLM? You can read better optimized documentation at /document/level-2/nginx_or_haproxy_tls_tunnel.md for this page in Markdown format</div><h1 id="nginx-或-haproxy-搭建-tls-隧道隐藏指纹" tabindex="-1">Nginx 或 Haproxy 搭建 TLS 隧道隐藏指纹 <a class="header-anchor" href="#nginx-或-haproxy-搭建-tls-隧道隐藏指纹" aria-label="Permalink to “Nginx 或 Haproxy 搭建 TLS 隧道隐藏指纹”">​</a></h1><p>Nginx 或 Haproxy 实现的 HTTPS 隧道、HTTP/2 over HTTPS 隧道、WebSocket over HTTP/2 over HTTPS 隧道、gRPC over HTTP/2 over HTTPS 隧道以及自签证书双端认证的 gRPC over HTTP/2 over HTTPS 隧道</p><h2 id="客户端服务端-nginx-构建-https-隧道隐藏指纹" tabindex="-1">客户端服务端 Nginx 构建 HTTPS 隧道隐藏指纹 <a class="header-anchor" href="#客户端服务端-nginx-构建-https-隧道隐藏指纹" aria-label="Permalink to “客户端服务端 Nginx 构建 HTTPS 隧道隐藏指纹”">​</a></h2><p>网路结构：</p><p>xray_client ---tcp--- nginx_client ---HTTPS--- nginx_sever ---tcp--- xray_server</p><h2 id="编译-nginx-with-stream" tabindex="-1">编译 nginx --with-stream <a class="header-anchor" href="#编译-nginx-with-stream" aria-label="Permalink to “编译 nginx --with-stream”">​</a></h2><p>在客户端及服务端均编译</p><p><code>curl -O -L http://nginx.org/download/nginx-1.22.1.tar.gz</code></p><p><code>tar -zxvf nginx-1.22.1.tar.gz</code></p><p><code>cd nginx-1.22.1</code></p><p><code>apt install gcc make</code> //编译依赖 gcc 以及 make</p><p><code>./configure --prefix=/usr/local/nginx --with-http_ssl_module --with-http_v2_module --with-stream --with-stream_ssl_module</code> //此步需要依赖一些库，根据报错安装相应 lib</p><p><code>make &amp;&amp; make install</code></p><p>编译之后 nginx 文件夹位于 <code>/usr/local/nginx</code></p><h2 id="配置-nginx" tabindex="-1">配置 nginx <a class="header-anchor" href="#配置-nginx" aria-label="Permalink to “配置 nginx”">​</a></h2><p>编辑 nginx 配置文件 nginx.conf</p><p><code>vim /usr/local/nginx/conf/nginx.conf</code></p><p>服务端加入如下配置</p><p>服务器申请证书不再赘述，参考<a href="./../level-0/ch06-certificates.html">白话文</a></p><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>stream {</span></span>
<span class="line"><span>    server {</span></span>
<span class="line"><span>        listen 443 ssl;</span></span>
<span class="line"><span>        listen [::]:443 ssl;</span></span>
<span class="line"><span>        ssl_protocols TLSv1.3;</span></span>
<span class="line"><span>        ssl_certificate /path/to/cert/domain.crt; # crt 文件位置</span></span>
<span class="line"><span>        ssl_certificate_key /path/to/cert/domain.key; # key 文件位置</span></span>
<span class="line"><span>        proxy_pass unix:/dev/shm/vless.sock; # 使用 domain socket</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br></div></div><div class="warning custom-block"><p class="custom-block-title">注意</p><p>stream 部分与 http 模块并列，客户端可删除 http 部分，服务端可删除或搭建网页伪装回落</p></div><p>客户端加入如下配置</p><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>stream {</span></span>
<span class="line"><span>    server {</span></span>
...
<span class="line"><span>        proxy_ssl_name yourdomain.domain; # 服务器域名</span></span>
<span class="line"><span>        proxy_pass ip:443; # 服务器 ip 形如 proxy_pass 6.6.6.6:443; 或 proxy_pass [2401:0:0::1]:443;</span></span>
<span class="line"><span>    }</span></span>
<span class="line"><span>}</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br></div></div><p>在 <code>/etc/systemd/system</code> 文件夹中创建 <code>nginx.service</code> 文件</p><p><code>vim /etc/systemd/system/nginx.service</code></p><p>写入如下</p><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>[Unit]</span></span>
...
<span class="line"><span style="--shiki-dark:#D4D4D4;--shiki-light:#000000;">    }</span></span>
<span class="line"><span style="--shiki-dark:#D4D4D4;--shiki-light:#000000;">  ]</span></span>
<span class="line"><span style="--shiki-dark:#D4D4D4;--shiki-light:#000000;">}</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br></div></div><p>客户端 xray 配置，此处以旁路由透明代理为例</p><div class="language-json line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang">json</span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span style="--shiki-dark:#D4D4D4;--shiki-light:#000000;">{</span></span>
<span class="line"><span style="--shiki-dark:#9CDCFE;--shiki-light:#0451A5;">  &quot;log&quot;</span><span style="--shiki-dark:#D4D4D4;--shiki-light:#000000;">: {</span></span>
<span class="line"><span style="--shiki-dark:#9CDCFE;--shiki-light:#0451A5;">    &quot;loglevel&quot;</span><span style="--shiki-dark:#D4D4D4;--shiki-light:#000000;">: </span><span style="--shiki-dark:#CE9178;--shiki-light:#A31515;">&quot;none&quot;</span></span>
<span class="line"><span style="--shiki-dark:#D4D4D4;--shiki-light:#000000;">  },</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span># 设置策略路由 v6</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span># VPS IP 直连</span></span>
<span class="line"><span>iptables -t mangle -A XRAY_MASK -d VSP_IPv4/32 -j RETURN</span></span>
<span class="line"><span>ip6tables -t mangle -A XRAY6_MASK -d VPS_IPv6/128 -j RETURN</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br></div></div><h2 id="客户端及服务端启动服务" tabindex="-1">客户端及服务端启动服务 <a class="header-anchor" href="#客户端及服务端启动服务" aria-label="Permalink to “客户端及服务端启动服务”">​</a></h2><p><code>systemctl restart xray</code></p><p><code>systemctl restart nginx</code></p><h2 id="结束" tabindex="-1">结束 <a class="header-anchor" href="#结束" aria-label="Permalink to “结束”">​</a></h2><h1 id="双端-haproxy-构建-https-隧道隐藏指纹" tabindex="-1">双端 Haproxy 构建 HTTPS 隧道隐藏指纹 <a class="header-anchor" href="#双端-haproxy-构建-https-隧道隐藏指纹" aria-label="Permalink to “双端 Haproxy 构建 HTTPS 隧道隐藏指纹”">​</a></h1><p>安装 Haproxy</p><p><code>pacman -Su haproxy</code> 或 <code>apt install haproxy</code></p><p>Haproxy 处理 ssl 需要 openssl 支持，检查 openssl 版本，必要时安装或更新</p><h2 id="https-隧道" tabindex="-1">HTTPS 隧道 <a class="header-anchor" href="#https-隧道" aria-label="Permalink to “HTTPS 隧道”">​</a></h2><p>前述 Nginx HTTPS 隧道 Hproxy 同样可以简单做到</p><p>网路结构：</p><p>xray_client ---tcp--- haproxy_client ---HTTPS--- haproxy_sever ---tcp--- xray_server</p><h3 id="haproxy-client-配置-运行前去掉注释" tabindex="-1">haproxy_client 配置 (运行前去掉注释) <a class="header-anchor" href="#haproxy-client-配置-运行前去掉注释" aria-label="Permalink to “haproxy_client 配置 (运行前去掉注释)”">​</a></h3><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>global</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>    # 隧道强制使用 TLS 1.3</span></span>
...
<span class="line"><span>    bind 127.0.0.1:6666 # 监听本机 6666 端口</span></span>
...
<span class="line"><span>    # 域名或 IP 均可以，若填域名建议在 hosts 中指定 IP 降低解析时间；alpn 与服务器协商，服务器端为 alpn h2,http1.1 时，客户端指定为 h2 则隧道为 HTTP2 方式连接，指定为 http1.1 为 HTTP 方式，双端均写优先 h2</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br></div></div><h3 id="haproxy-server-配置-运行前去掉注释" tabindex="-1">haproxy_server 配置 (运行前去掉注释) <a class="header-anchor" href="#haproxy-server-配置-运行前去掉注释" aria-label="Permalink to “haproxy_server 配置 (运行前去掉注释)”">​</a></h3><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>global</span></span>
<span class="line"><span>    log /dev/log local0 alert</span></span>
<span class="line"><span>    log /dev/log local1 alert</span></span>
<span class="line"><span>    stats socket /dev/shm/admin.sock mode 660 level admin expose-fd listeners</span></span>
<span class="line"><span>    stats timeout 30s</span></span>
...
<span class="line"><span>    daemon</span></span>
<span class="line"><span></span></span>
<span class="line"><span>    # 指定安全套件并指定 ssl 版本最低 1.2 增加真实性</span></span>
...
<span class="line"><span>    bind :::443 ssl crt /path/to/pem alpn h2,http/1.1 # haproxy 使用 pem 进行 ssl 解密，pem 由 cat www.example.com.crt www.example.com.key &gt; www.example.com.pem 获得</span></span>
...
<span class="line"><span>    server xray /dev/shm/vless.sock # 支持 abstract 格式： &quot;abns@vless.sock&quot; ；loopback 方式：127.0.0.1:6666</span></span>
<span class="line"><span></span></span>
<span class="line"><span>backend web</span></span>
<span class="line"><span>    server web /dev/shm/h1h2c.sock # 回落到网页</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br><span class="line-number">32</span><br><span class="line-number">33</span><br></div></div><h3 id="xray-配置-1" tabindex="-1">xray 配置 <a class="header-anchor" href="#xray-配置-1" aria-label="Permalink to “xray 配置”">​</a></h3><p>同上 nginx 部分：最简单的 TCP 配置，可搭配任意协议，建议使用 VLESS+TCP 无需多余加密，参考文档或其他示例</p><h2 id="websocket-over-http-2" tabindex="-1">WebSocket over HTTP/2 <a class="header-anchor" href="#websocket-over-http-2" aria-label="Permalink to “WebSocket over HTTP/2”">​</a></h2><p>Haproxy 支持 HTTP/2 的 h2c 进站及出站</p><p>然而援引 xray 文档 HTTP/2 的说明</p><p>“由 HTTP/2 的建议，客户端和服务器必须同时开启 TLS 才可以正常使用这个传输方式。...... 当前版本的 HTTP/2 的传输方式并不强制要求入站（服务端）有 TLS 配置。”</p><p>即入站可以使用 h2c，出站并不支持 h2c。因此无法使用 xray_client ---h2c--- haproxy_client ---HTTP/2+TLS--- haproxy_sever ---h2c--- xray_server</p><p>但是可以通过 ws 偷个鸡，Haproxy 支持 ws over HTTP/2</p><p>则网络结构：xray_client ---ws--- haproxy_client ---ws over HTTP/2 over HTTPS--- haproxy_sever ---ws--- xray_server</p><h3 id="haproxy-client-配置" tabindex="-1">haproxy_client 配置 <a class="header-anchor" href="#haproxy-client-配置" aria-label="Permalink to “haproxy_client 配置”">​</a></h3><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>global</span></span>
<span class="line"><span>    log /dev/log local0 alert</span></span>
<span class="line"><span>    log /dev/log local1 alert</span></span>
<span class="line"><span>    stats socket /dev/shm/admin.sock mode 660 level admin expose-fd listeners</span></span>
<span class="line"><span>    stats timeout 30s</span></span>
...
<span class="line"><span>    daemon</span></span>
<span class="line"><span></span></span>
<span class="line"><span>    # 调整 HTTP/2 的性能，当遇到 HTTP/2 性能问题时都可以设置相关项，更多设置见 Haproxy 文档 tune.h2 部分 https://docs.haproxy.org/2.7/configuration.html</span></span>
<span class="line"><span>    tune.h2.initial-window-size 536870912 # 初始窗口大小，建议设置，默认值 65536 单位 byte，此值在突发大流量情况下需要一定加载时间，建议根据网速调整</span></span>
<span class="line"><span>    tune.h2.max-concurrent-streams 512 # 复用线路数，可根据情况设置，默认值 100，一般不用设置(官方不建议改动)</span></span>
<span class="line"><span></span></span>
<span class="line"><span>    ssl-default-server-options ssl-min-ver TLSv1.3</span></span>
<span class="line"><span></span></span>
<span class="line"><span>defaults</span></span>
...
<span class="line"><span>    daemon</span></span>
<span class="line"><span></span></span>
<span class="line"><span>    # 客户端配置即可，服务端配置也无妨</span></span>
...
<span class="line"><span>    # haproxy 使用 http 模式可以根据 path 分流</span></span>
<span class="line"><span></span></span>
<span class="line"><span>backend xray</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>backend web</span></span>
<span class="line"><span>    server web /dev/shm/h1h2c.sock</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br><span class="line-number">32</span><br><span class="line-number">33</span><br><span class="line-number">34</span><br><span class="line-number">35</span><br><span class="line-number">36</span><br><span class="line-number">37</span><br><span class="line-number">38</span><br><span class="line-number">39</span><br><span class="line-number">40</span><br><span class="line-number">41</span><br><span class="line-number">42</span><br><span class="line-number">43</span><br><span class="line-number">44</span><br><span class="line-number">45</span><br><span class="line-number">46</span><br><span class="line-number">47</span><br></div></div><h3 id="xray-配置-2" tabindex="-1">xray 配置 <a class="header-anchor" href="#xray-配置-2" aria-label="Permalink to “xray 配置”">​</a></h3><p>简单的 websocket 配置即可，无需 TLS， 配置见 xray 文档示例，配置 &quot;path&quot; 可以用于服务端 haproxy 分流（客户端有分流需求同样可以通过客户端 haproxy 进行，原理类似，参考服务端的 path 分流配置）</p><h2 id="grpc-over-http-2" tabindex="-1">gRPC over HTTP/2 <a class="header-anchor" href="#grpc-over-http-2" aria-label="Permalink to “gRPC over HTTP/2”">​</a></h2><p>虽然双端的 h2c 不行，但是 gRPC 不要求必须 TLS，直接冲</p><p>网络结构：xray_client ---gRPC h2c--- haproxy_client ---gRPC over HTTP/2 over HTTPS--- haproxy_sever ---gRPC h2c--- xray_server</p><h3 id="haproxy-client-配置-1" tabindex="-1">haproxy_client 配置 <a class="header-anchor" href="#haproxy-client-配置-1" aria-label="Permalink to “haproxy_client 配置”">​</a></h3><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>global</span></span>
<span class="line"><span>    log /dev/log local0 alert</span></span>
<span class="line"><span>    log /dev/log local1 alert</span></span>
<span class="line"><span>    stats socket /dev/shm/admin.sock mode 660 level admin expose-fd listeners</span></span>
<span class="line"><span>    stats timeout 30s</span></span>
...
<span class="line"><span>frontend tls-in</span></span>
<span class="line"><span>    bind :::443 ssl crt /path/to/pem alpn h2,http/1.1</span></span>
<span class="line"><span>    use_backend xray if { ssl_fc_alpn -i h2 } { path_beg /tunnel } # xray gRPC 中配置的 &quot;serviceName&quot; 在 harpoxy 中可以使用 path 进行分流，为方便使用 &quot;multiMode&quot;，使用 path_beg 参数匹配路径</span></span>
<span class="line"><span>    use_backend server1 if { ssl_fc_alpn -i h2 } { path_beg /path1 }</span></span>
<span class="line"><span>    use_backend server2 if { ssl_fc_alpn -i h2 } { path_beg /path2 }</span></span>
<span class="line"><span>    use_backend server3 if { ssl_fc_alpn -i h2 } { path_beg /path3 }</span></span>
<span class="line"><span>    default_backend web</span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>backend web</span></span>
<span class="line"><span>    server web /dev/shm/h1h2c.sock</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br><span class="line-number">32</span><br><span class="line-number">33</span><br><span class="line-number">34</span><br><span class="line-number">35</span><br><span class="line-number">36</span><br><span class="line-number">37</span><br><span class="line-number">38</span><br><span class="line-number">39</span><br><span class="line-number">40</span><br><span class="line-number">41</span><br><span class="line-number">42</span><br><span class="line-number">43</span><br><span class="line-number">44</span><br><span class="line-number">45</span><br></div></div><h3 id="xray-配置-3" tabindex="-1">xray 配置 <a class="header-anchor" href="#xray-配置-3" aria-label="Permalink to “xray 配置”">​</a></h3><p>简单的 gRPC 配置，无需 TLS，配置见文档，配置的 serviceName 可用于分流。</p><h1 id="haproxy-使用自签证书进行双端认证-grpc-示例" tabindex="-1">Haproxy 使用自签证书进行双端认证（gRPC 示例） <a class="header-anchor" href="#haproxy-使用自签证书进行双端认证-grpc-示例" aria-label="Permalink to “Haproxy 使用自签证书进行双端认证（gRPC 示例）”">​</a></h1><p>这里使用自签证书双端认证加强隧道安全性（但会牺牲一点延迟，不过使用 gRPC 后感知不强），而服务端同时处理信任的证书和自签名证书，并据此分流伪装网站和隧道流量</p><p>其中 www.example.com 为伪装站信任证书（如白话文中申请的证书）</p><p>tunnel.example.com 为自签证书网址，自签证书可以参考 <a href="https://learn.microsoft.com/zh-cn/azure/application-gateway/self-signed-certificates" target="_blank" rel="noreferrer">https://learn.microsoft.com/zh-cn/azure/application-gateway/self-signed-certificates</a></p><p>根证书 ca.crt 服务器证书 server.crt 服务器密钥 server.key</p><p>至少需要生成一个 server.pem，客户端可以同样使用此证书用于双端认证；或者生成两个证书，一个 client，一个 server，用于双端认证</p><p>需准备 fullchain.crt 用于认证（ cat server.crt ca.crt &gt; fullchain.crt ），server.pem （ cat server.crt server.key ca.crt &gt; server.pem ）用于解密</p><h3 id="haproxy-client-配置-2" tabindex="-1">haproxy_client 配置 <a class="header-anchor" href="#haproxy-client-配置-2" aria-label="Permalink to “haproxy_client 配置”">​</a></h3><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>global</span></span>
<span class="line"><span>    log /dev/log local0 alert</span></span>
<span class="line"><span>    log /dev/log local1 alert</span></span>
<span class="line"><span>    stats socket /dev/shm/admin.sock mode 660 level admin expose-fd listeners</span></span>
<span class="line"><span>    stats timeout 30s</span></span>
...
<span class="line"><span>    # 网址自定义，和自签证书一致即可，hosts 中配置 IP 解析，sni 的 str 设定 sni，用于服务端识别</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br></div></div><h3 id="haproxy-server-配置-2" tabindex="-1">haproxy_server 配置 <a class="header-anchor" href="#haproxy-server-配置-2" aria-label="Permalink to “haproxy_server 配置”">​</a></h3><div class="language- line-numbers-mode"><button title="Copy code" data-copied="Copied" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes dark-plus light-plus" style="--shiki-dark:#D4D4D4;--shiki-light:#000000;--shiki-dark-bg:#1E1E1E;--shiki-light-bg:#FFFFFF;" tabindex="0" dir="ltr"><code><span class="line"><span>global</span></span>
<span class="line"><span>    log /dev/log local0 alert</span></span>
<span class="line"><span>    log /dev/log local1 alert</span></span>
<span class="line"><span>    stats socket /dev/shm/admin.sock mode 660 level admin expose-fd listeners</span></span>
<span class="line"><span>    stats timeout 30s</span></span>
...
<span class="line"><span>    default_backend web</span></span>
<span class="line"><span>    # Haproxy 支持多个 pem 解密</span></span>
<span class="line"><span>    # 可根据多个客户端的不同 sni 分流，也可以 path 分流，方式多样，更多 acl 见 Haproxy 文档</span></span>
<span class="line"><span></span></span>
<span class="line"><span>backend xray</span></span>
<span class="line"><span>    server xray abns@vless.sock proto h2</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span></span></span>
<span class="line"><span>backend web</span></span>
<span class="line"><span>    server web /dev/shm/h1h2c.sock</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br><span class="line-number">16</span><br><span class="line-number">17</span><br><span class="line-number">18</span><br><span class="line-number">19</span><br><span class="line-number">20</span><br><span class="line-number">21</span><br><span class="line-number">22</span><br><span class="line-number">23</span><br><span class="line-number">24</span><br><span class="line-number">25</span><br><span class="line-number">26</span><br><span class="line-number">27</span><br><span class="line-number">28</span><br><span class="line-number">29</span><br><span class="line-number">30</span><br><span class="line-number">31</span><br><span class="line-number">32</span><br><span class="line-number">33</span><br><span class="line-number">34</span><br><span class="line-number">35</span><br><span class="line-number">36</span><br><span class="line-number">37</span><br><span class="line-number">38</span><br><span class="line-number">39</span><br><span class="line-number">40</span><br><span class="line-number">41</span><br><span class="line-number">42</span><br><span class="line-number">43</span><br><span class="line-number">44</span><br><span class="line-number">45</span><br><span class="line-number">46</span><br><span class="line-number">47</span><br></div></div><h3 id="xray-配置-4" tabindex="-1">xray 配置 <a class="header-anchor" href="#xray-配置-4" aria-label="Permalink to “xray 配置”">​</a></h3><p>简单的 gRPC 配置，无需 TLS，配置见文档，配置的 serviceName 可用于分流。</p>`,90)]])}var s=n(a,[[`render`,o]]);export{i as __pageData,s as default};