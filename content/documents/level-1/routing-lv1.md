...
请大家注意看，我上面每一条路由规则，都是一个独立的匹配元素，这样才能确保生效。

新人在自定义路由规则时，经常会犯的一个错误是，在一条规则内匹配多种不同元素。比如，他希望实现的配置是：
...
{{% notice warning  %}}
**注意：** **同一个规则之内，各个元素需要同时成立，才会匹配成功**，逻辑关系是 `AND`，而不是 `OR`。
{{% /notice %}}
...
其实，第6点已经是我整理过的规则了，原则就是【相同的匹配元素可以合并，不同的匹配元素保持独立】。
