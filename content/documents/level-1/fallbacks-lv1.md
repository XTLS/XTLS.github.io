...
2. <font size=4>**`Xray` 的入站协议 (`inbound protocol`) 是 `vless`**</font>
    ...
    {{% notice warning  %}}
**注：** `VLESS`是给 `xray` 及 `v2fly` 引入回落功能的新型轻量协议。目前，xray中的 `trojan` 协议也已完整支持回落功能。
{{% /notice %}}

3. <font size=4>**回落目标端口 (`fallback dest`) 是 `8080`**</font>

    <font size=2>`Xray` 接受 `443` 端口的访问流量后，属于 `vless` 协议的流量、由 `Xray` 进行内部处理并转发
    非 `vless` 协议的流量，转发至 `8080` 端口。</font>
...
4. <font size=4>**回落给 `8080` 端口的流量，由后续配置处理**</font>
...
- 正常的网站的回应，一定不会有 `xray`, `v2fly`, `shadowsocks` 等代理工具的特征
- 正常的网站的回应，一定会有 `Nginx`, `Apache`, `MySQL`, `PHP` 等网络运维软件的特征


当我们给 `Xray` 提供了回落的能力后（如上例，回落给了 `Nginx`），面对任何用来探测的请求，产生的结果是：

- 探测流量无法掌握你的 `VLESS` 要素，故都会被回落至 `Nginx`
- 因为探测流量根本没有进入 `Xray`，所以VPS的回应一定不会有 `Xray` 的特征
- 因为探测流量全都回落进入 `Nginx` ，所以VPS的回应一定会有 `Nginx` 的特征

至此，逻辑上就完美的解决了服务器被 **【主动探测】** 的安全隐患。当然了，代码实现上肯定会复杂很多。
...
基于这个开发理念，【回落】功能才逐渐成长为现在的完全体，即完成了 `纯伪装 --> ws分流 --> 多协议多特征分流` 的进化，甚至完全替代了以前要用Web服务器、其他工具才能完成的分流的功能。且由于上述的处理都在首包判断阶段快速完成，几乎完全没有过程损耗。

**因此，现在 `Xray` 完整的回落功能，同时具备下述三个属性：**
...
5. <font size=4>**网页回落不见了！**</font>

    没错，聪明的同学应该发现了，防御【主动探测】的 `nginx回落` 不见了！！！这是为什么呢？别急，我们继续分析：
...
1. 后续处理回落至 `1310` 端口的流量，按照下面的配置验证、处理：
    ...
    看，神奇的事情发生了，这里又出现了一个 `fallbacks`。前面已经说过，`trojan`也具有完整的回落能力，所以，此时 `trojan` 协议可以再次做判断和回落。这也就是传说中的套娃回落了：
    
    - `tronan` 协议的流入 `Xray` 中做后续处理
    - 非 `trojan` 协议的流量，转发至 `80` 端口，【主动探测】的防御，完成！

2. 后续处理回落至 `1234` 端口的流量，仔细看！它其实是 `vless+ws+tls`：
    ```
    {
        "port": 1234,
        "listen": "127.0.0.1",
